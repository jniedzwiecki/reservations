...
public class EventController {
    ...
    public ResponseEntity<List<EventResponse>> getAllEvents(final Authentication authentication) {
        final boolean customerView = authentication.getAuthorities().stream()
                .anyMatch(auth -> auth.getAuthority().equals("ROLE_CUSTOMER"));
        final List<EventResponse> events = eventService.getAllEvents(customerView);
    ...
    public ResponseEntity<EventResponse> createEvent(@Valid @RequestBody final CreateEventRequest request) {
        final EventResponse event = eventService.createEvent(request);
    ...
    public ResponseEntity<EventResponse> updateEvent(
        ...
        final EventResponse event = eventService.updateEvent(id, request);
        return ResponseEntity.ok(event);
    }
    ...
    public ResponseEntity<EventResponse> updateEventStatus(
        ...
        final EventResponse event = eventService.updateEventStatus(id, request);
        return ResponseEntity.ok(event);
    }
    ...
    public ResponseEntity<EventSalesResponse> getEventSales(@PathVariable final UUID id) {
        final EventSalesResponse sales = eventService.getEventSales(id);
